...
queueEvents.on('completed', async ({ jobId, returnvalue }) => {
  logger.info(`QueueEvents: Job ${jobId} concluído.`);
   const task = await dbGet('SELECT task_id, user_id, status FROM tasks WHERE job_id = ?', [jobId]); // Usa dbGet placeholder
   if(task && task.status !== 'completed' && task.status !== 'failed') { 
       ...
       await dbRun("UPDATE tasks SET status = 'completed', result = ?, updated_at = ? WHERE task_id = ?", [JSON.stringify(returnvalue), Date.now(), task.task_id]); // Usa dbRun placeholder
   } else if (!task) {
   ...
   logger.error(`QueueEvents: Job ${jobId} falhou: ${failedReason}`);
    const task = await dbGet('SELECT task_id, user_id, status FROM tasks WHERE job_id = ?', [jobId]); // Usa dbGet placeholder
     if(task && task.status !== 'failed') { 
         ...
         await dbRun("UPDATE tasks SET status = 'failed', result = ?, updated_at = ? WHERE task_id = ?", [JSON.stringify({ error: failedReason }), Date.now(), task.task_id]); // Usa dbRun placeholder
     } else if (!task) {
